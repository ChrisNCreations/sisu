...
Canonical UI context. Read this before any `sisu/app` work. Protocol math lives in the contracts and the architecture grill; this file is the product surface only.
...
- Next.js App Router at `sisu/app`
...
- Quote / ship / swap / dock / decode live in `sisu/app/lib`, not a published SDK
...
`scripts/setup-ui.ts` leaves the local node UI-ready: deploy, fund accounts 0 and 1, ship one 50/50 strategy, write `app/lib/deployment.json`. Swap works before anyone uses Create.
